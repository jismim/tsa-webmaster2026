/* ============================================================
   CareMap Morris — Donate & Volunteer Page JS
   Handles: hero load-in, scroll reveal, card filtering,
            tab switching, chip filters, footer year
============================================================ */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var DATA = [
    ...
    {id:28, kind:"donate & volunteer", title:"Legal Services of Northwest Jersey",    desc:"Provides free civil legal assistance to eligible low-income residents of Morris County.",                                    age:["adult","family","senior"], location:"morristown",    services:["legal"],                       link:"https://www.lsnwj.org/",                                                                     phone:"(973) 285-6911", address:"30 Schuyler Pl, Morristown, NJ 07960"}
  ];
  ...
  function render() {
    ...
    var location = filterLocation.value.toLowerCase();
    ...
    var filtered = DATA.filter(function (item) {
      ...
      if (activeServices.length && !activeServices.every(function (s) { return item.services.includes(s); })) return false;
      return true;
    });

    cardsList.innerHTML = filtered.map(function (item) {
      var badgeClass = item.kind === 'donate' ? 'badge-donate' : item.kind === 'volunteer' ? 'badge-volunteer' : 'badge-both';
      var badgeLabel = item.kind === 'donate & volunteer' ? 'Donate & Volunteer' : item.kind.charAt(0).toUpperCase() + item.kind.slice(1);
      var pills      = item.services.map(function (s) { return '<span class="card-pill">' + s + '</span>'; }).join('');
  ...
  tabBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      if (heading) {
        if      (activeTab === 'donate')    heading.textContent = 'Donate Opportunities';
        else if (activeTab === 'volunteer') heading.textContent = 'Volunteer Opportunities';
        else                                heading.textContent = 'All Opportunities';
      }
  ...
  chips.forEach(function (chip) {
    chip.addEventListener('click', function () {
      ...
      activeServices = activeServices.includes(val)
        ? activeServices.filter(function (s) { return s !== val; })
        : activeServices.concat([val]);
      render();
    });
  });

  /* ── 9. Age & location dropdowns ── */
